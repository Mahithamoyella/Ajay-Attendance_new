...
app.post('/api/punch', async (req, res) => {
    ...
    try {
        ...
        const latestRecord = await pool.query(
            'SELECT status, timestamp FROM new_attendance WHERE employee_id = $1 AND date = CURRENT_DATE ORDER BY timestamp DESC LIMIT 1',
            [employeeId]
        );
        ...
        const result = await pool.query(
            'INSERT INTO new_attendance (employee_id, status, timestamp, date, hours_worked, attendance_status) VALUES ($1, $2, $3, CURRENT_DATE, $4, $5) RETURNING *',
...
app.delete('/api/records', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'DELETE FROM new_attendance WHERE employee_id = ANY($1) RETURNING *',
            [employeeIds]
        );
...
// Start server
app.listen(port, () => {
    console.log(`Server running at http://44.203.74.159:${port}`);
});